...
前日は @<href>{https://twitter.com/Tukimikage,Tukimikage}さんの「@<href>{http://think-sv.net/blog/?p=1290,続・Cloudn_PaaSでFuelPHPを動かしてみた}」でした。
...
FuelPHPのmoduleについては、ここでは割愛させて頂きます。@<br>{}
...
@<tt>{/admin/**/*}なURLへのリクエストを、まずはController_Adminで受け取り、@<br>{}
...
モジュール間の連携処理をapp内でゴニョゴニョ実装する手もありますが、@<br>{}
 こちらのほうがソースの構成がスッキリすると思います。

== adminモジュールとuserモジュールの疎結合


userモジュールは、adminモジュールの提供する機能を存分に利用してコーディングを行う事ができますが、@<br>{}
 adminモジュールからは、userモジュールを前提としたコーディングはしない方がよいでしょう。


この点については、設定ファイルやPHPのinterfaceを活用して、@<br>{}
 モジュール間の実装インターフェースの規約が必要になると思います。


実際、自分の仕事では、adminモジュールの管理画面テンプレートへ組み込むViewの受け渡し方法として、@<br>{}
...
ここまでお読み頂き、ありがとうございます。@<br>{}
 少しでも皆様のお役に立つことが出来れば幸いです。


次回は、@<href>{http://twitter.com/suno88,suno88}さんの「レンタルサーバー XREA/CORESERVER で FuelPHP を使う [実践編]」です。
