...
= @<href>{http://blog.omoon.org/20131219/925,#FuelPHP 5 分で API を実装するチュートリアル（スクリーンキャストあり）}


** 2013/12/19


@<href>{http://atnd.org/events/45096,FuelPHP Advent Calendar 2013} の 19 日目です。昨日は、@<href>{https://twitter.com/madmamor,@madmamor} さんの「@<href>{http://madroom-project.blogspot.jp/2013/12/fac20131218.html,FuelPHPとMongoDBとTraceKitでJavaScriptのエラー情報を収集してみる}」でした。
...
FuelPHP には @<tt>{Controller_Rest} というものが用意されていて、API を作るのがとても簡単なんですよー、とはよく言われるところですが、実際にどれぐらい簡単なのかやってみました。
 ...
 * FuelPHP 1.7
 * データベースに mysql を利用
 * mysql の host は localhost、port は 3306、ユーザ、パスワードともに @<tt>{root}、データベース名は @<tt>{fuel_dev}（FuelPHP インストール時の@<tt>{development}環境でのデフォルト設定です）
...
//}](http://www.flickr.com/photos/omoon/11446914955)
...
ここも@<tt>{oil generate}でさくっと行きたいところですが、ざっと見た感じでは、Controller_Rest 用のオプションなどがあまり用意されていないようでしたので、ある程度まで作ってちょこっと修正する方針で行きます。
 ...
 * @<tt>{protected $format = 'json';} でデフォルトフォーマットを JSON に
 * @<tt>{public function get_read()} で @<tt>{read}を GET メソッドで受け付けるように
 * @<tt>{public function post_write()} で @<tt>{write}を POST メソッドで受け付けるように
 ...
 * @<href>{http://d.hatena.ne.jp/Kenji_s/20130121/fuel_web_api_5_min,FuelPHP を使って 5分で Web API を作成する}
...
明日は、@<href>{https://twitter.com/Altsencturely,@Altsencturely} さんです。
