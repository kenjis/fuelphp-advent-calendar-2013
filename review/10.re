...
@<href>{http://atnd.org/events/45096,FuelPHP advent Calendar 2013}の10日目です。昨日は@<href>{http://blog.a-way-out.net/blog/2013/12/09/fuelphp-aspectmock/,AspectMockでFuelPHPのアプリを100％テスト可能にする}(@<href>{https://twitter.com/kenji_s,@kenji_s})でした。
...
アプリケーションの処理の流れ: 青丸がトリガー@<br>{}
//image[fuelphp_event1][アプリケーションの処理の流れ]{
//}


イベントを実行: 処理途中の各トリガーの所では、登録されているイベントがあれば実行する@<br>{}
//image[fuelphp_event2][イベントを実行]{
//}


コードは点線部分で分離されている: アプリケーション自体のコードに手を入れずに、機能追加／変更する@<br>{}
//image[fuelphp_event3][コードは点線部分で分離されている]{
//}
...
1.は、@<tt>{app/config}に@<tt>{event.php}に書く方法です。詳細は@<href>{http://fuelphp.jp/docs/1.7/classes/event.html,Event クラス(FuelPHP1.7)}をごらんください。


​2. は、@<tt>{register}メソッドを使う方法です。今回はこちらを使います。@<href>{http://fuelphp.jp/docs/1.7/classes/event.html,Event クラス(FuelPHP1.7)}では、@<tt>{user_login}に@<tt>{Class::method}処理を追加する例が掲載されています。
...
@<tt>{WARNING - 2013-12-09 09:15:23 --> Login Fail.  Email: email@example.com  Password: p@ssw0rd  IP: 127.0.0.1}
...
@<href>{http://fuelphp.jp/docs/1.7/classes/event.html,Event クラス(FuelPHP1.7)}によると、デフォルトで用意されているイベントは、@<tt>{app_created}、@<tt>{request_created}、@<tt>{request_started}、@<tt>{controller_started}、@<tt>{controller_finished}、@<tt>{response_created}、@<tt>{request_finished}、@<tt>{shutdown}です。これらのトリガーに処理を追加することができます。

=== アプリケーションにトリガーを用意する


コアに用意されているトリガーを使うだけではなく、アプリケーションにもトリガーを用意することができます。@<href>{#trigger,アプリケーションの処理の流れ: 青丸がトリガー}の、青丸を作る作業です。コード中の適当な箇所に、@<tt>{Event::trigger(トリガー名)}と書けばOKです。
...
明日は、「@<href>{http://chatii.net/articles/php/2013/12/fuelphp-composer.html,FuelPHP をもっと Composer で使う}」(@<href>{https://twitter.com/chatii0079,chatii0079})です。お楽しみに。
