...
@<href>{http://atnd.org/events/45096,FuelPHP Advent Calendar 2013}の8日目です。


CMS系のサイト構築は大体どのフレームワークでも問題ないのですが、基幹業務チックな機能要件があると、様々なモデルの組み合わせで入力値の検証をしたりだとか、項目間において依存性のあるチェックだとかが頻発します。@<br>{}
...
@<href>{http://fuelphp.jp/docs/1.7/classes/validation/validation.html,公式ドキュメント(日本語訳)}@<br>{}
 デフォルトの入力検証含め、こちらに書いてある内容でほとんどの入力値の検証は可能です。

== 2.複数の任意のパラメータを用いて検証したい場合


複数のポストされたパラメータを用いた検証を行いたい場合、拡張Validationを書きますが、@<br>{}
...
答えはノー。再利用性のあるもの(汎用的な独自バリデーション)はクラスにしておいた方が後々使い回しがききますが、例えばログイン検証なんかは再利用しませんよね？@<br>{}
...
Closureで複数のルールを定義するとValidation::active()->set_messageで定義したメッセージがおかしなことになります。以下のサンプルを見てください。@<br>{}
...
後に定義した方のメッセージが先に定義したclosureのメッセージを上書きしてしまうんですね。@<br>{}
...
見てわかるとおりClosureのルールをキーとClosureの連想配列でadd_ruleに渡してあげます。@<br>{}
...
コミュニティも盛り上がり、各地で実績が増え、弊社でもFuelPHPが定着化してきました。@<br>{}
